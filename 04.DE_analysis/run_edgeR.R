...
for (i in 1:nrow(contrasts)) {
  ...
  write.table(result_table, file=paste0(treatment, "_vs_", control, ".edgeR.DE_results.txt"), sep = "\t", quote = F, row.names = T)
  write.table(rnaseqMatrix, file=paste0(treatment, "_vs_", control, ".edgeR.count_matrix.txt"), sep = "\t", quote = F, row.names = T)  
}
